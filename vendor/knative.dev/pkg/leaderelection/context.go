...
var errOdinalNotFound = errors.New("odinal not found from the name")
...
// LeaderElectors with the provided component configuration acquiring resource
// locks via the provided kubernetes client.
func WithStatefulSetLeaderElectorBuilder(ctx context.Context, kc kubernetes.Interface, cc ComponentConfig) context.Context {
...
func (b *statefulSetBuilder) BuildElector(ctx context.Context, la reconciler.LeaderAware, name string, enq func(reconciler.Bucket, types.NamespacedName)) (Elector, error) {
	logger := logging.FromContext(ctx)

	buckets := make([]Elector, 0)
	ordinal, err := extraOrdinalFromPodName()
	if err != nil {
		return nil, err
	}
	for i := uint32(0); i < b.lec.Buckets; i++ {
		if uint64(i)%3 == ordinal {
			bkt := &bucket{
				component: b.lec.Component,
				name:      name,
				index:     i,
			...
			logger.Infof("%s will run in leader-elected mode in pod %q", bkt.Name(), os.Getenv("POD_NAME"))
			buckets = append(buckets, &unopposedElector{bkt: bkt, la: la, enq: enq})
		}
	}
	return &runAll{les: buckets}, nil
}
